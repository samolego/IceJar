...
public interface IceJarPlayer {
    ...
    static void broadcast(ServerPlayer player, Check failedCheck) {
        ...
        final MutableComponent report = ID.copy()
                ...
                Component.translatable(player.getGameProfile().getName())
                        .withStyle(ChatFormatting.GOLD),
                Component.translatable(type.toString().toLowerCase())
        ...
        .append(Component.translatable(" (hover)")
                        .withStyle(ChatFormatting.ITALIC)
                        .withStyle(ChatFormatting.BLUE)
                        .withStyle(s -> s
                            .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                                    Component.translatable("Violation level: %s\nCheck violation level: %s",
                                            Component.translatable(String.format("%.2f", ((IceJarPlayer) player).ij$getViolationLevel()))
                                                    .withStyle(ChatFormatting.GOLD),
                                            Component.translatable(String.format("%.2f", failedCheck.getViolationLevel()))
                                                    .withStyle(ChatFormatting.YELLOW)
                                    )
