...
public class CombinedListener extends CheckListener {
    ...
    public void onPlayerJoin(final ServerPlayerJoinEvent event) {
    ...
    @EventHandler(priority = EventPriority.LOWEST)
    public void onPlayerLeave(final ServerPlayerQuitEvent event) {
        final ServerPlayer player = event.getPlayer();
        final IPlayerData pData = DataManager.getPlayerData(player);
        if (!pData.isCheckActive(CheckType.COMBINED, player)) return;
        final CombinedData data = pData.getGenericInstance(CombinedData.class);
    ...
    public void onPlayerToggleSprintHighest(final ServerPlayerToggleSprintEvent event) {
    ...
    @EventHandler(priority=EventPriority.LOWEST)
    public void onPlayerFish(final ServerPlayerFishEvent event){
        // Check also in case of cancelled events.
        final ServerPlayer player = event.getPlayer();
